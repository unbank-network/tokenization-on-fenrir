...
import VotingWallet from 'components/v2/Vote/VotingWallet';
...
const xvsTokenAddress = getToken('fnr').address;
console.log('xvsTokenAddress+++++', xvsTokenAddress);
...
function Vote() {
  ...
  const xvsTokenContract = useTokenContract('fnr');
  ...
  const updateBalance = async () => {
    if (account) {
      ...
      const [balanceTemp] = await Promise.all([
  ...
  const getVoteInfo = async () => {
    ...
    const [compInitialIndex, compAccrued] = await Promise.all([
      comptrollerContract.methods.compInitialIndex().call(),
      comptrollerContract.methods.compAccrued(myAddress).call(),
    ...
    await Promise.all(
      Object.values(VBEP_TOKENS).map(async item => {
        ...
        const [
          ...
          comptrollerContract.methods.compSupplyState(item.address).call(),
          vBepContract.methods.balanceOf(myAddress).call(),
          comptrollerContract.methods.compBorrowState(item.address).call(),
          comptrollerContract.methods.compBorrowerIndex(item.address, myAddress).call(),
        ...
        let supplierIndex = await Promise.resolve(
          comptrollerContract.methods.compSupplierIndex(item.address, myAddress).call(),
        );
    ...
    venusEarned = venusEarned.plus(compAccrued).dividedBy(1e18).dp(8, 1).toString(10);
