  ...
  return (
    <Paper css={styles.coinInfo} className={className}>
      ...
      <Typography variant="small1" noWrap component="span">
        {format(balance.dp(2, 1).toString(10))}
        {' '}
        {coin.toUpperCase()}
      </Typography>
    </Paper>
...
export const FnrCoinInfo = ({ className }: { className?: string }) => {
  const { userXvsBalance } = useMarketsUser();
  return <CoinInfo balance={userXvsBalance} coin="fnr" className={className} />;
};
