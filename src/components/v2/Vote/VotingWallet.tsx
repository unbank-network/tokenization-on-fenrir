...
import coinImg from 'assets/img/coins/fnr.svg';
...
function VotingWallet({ balance, earnedBalance }: VotingWalletProps) {
  ...
  const handleCollect = async () => {
    if (!account) {
      console.error('Cannot collect fnr tokens: ', 'account undefined');
      return;
    }
    ...
    if (+earnedBalance !== 0) {
      setIsLoading(true);
      try {
        await comptrollerContract.methods['claimComp(address)'](account.address).send({
          from: account.address,
        });
      } catch (error) {
        console.log('claim FNR error :>> ', error);
      }
      setIsLoading(false);
    }
  };
    ...
    <Card>
      <VotingWalletWrapper>
        <div className="flex align-center header">
          <p className="title">FNR Rewards</p>
        </div>
        <div className="flex flex-column content content-border-bottom">
          <p className="content-label">FNR Balance</p>
          ...
          {!isLoadingEarn && (
            <div className="flex align-center just-between">
              <div className="flex flex-column">
                <p className="content-label">FNR Earned</p>
                <div className="flex align-center">
                  <img src={coinImg} alt="coin" />
