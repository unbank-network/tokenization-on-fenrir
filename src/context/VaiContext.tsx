...
const VaiContextProvider = ({ children }: $TSFixMe) => {
  const [userVaiMinted] = useState(new BigNumber(0));
  const [userVaiBalance] = useState(new BigNumber(0));
  const [userVaiEnabled] = useState(false);
  const [mintableVai] = useState(new BigNumber(0));
  ...
  }, [fastRefresh, vaiControllerContract, comptrollerContract, account]);
